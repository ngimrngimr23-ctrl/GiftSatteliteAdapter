...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "/setmarkup <acc> <%> — сменить наценку над floor для одного аккаунта\n"
        "/setmarkupall <%> — сменить наценку над floor сразу для всех аккаунтов\n"
        "/forceupdate <acc> — пересчитать цены сейчас для одного аккаунта\n"
        "/forceupdateall — пересчитать цены сейчас для всех аккаунтов\n"
...
async def cmd_setmarkup_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Меняет наценку сразу для всех аккаунтов, без указания имени."""
    if not authorized(update):
        return
    accounts = context.bot_data["accounts"]
    if not accounts:
        ...
        return
    if not context.args:
        await update.message.reply_text("Использование: /setmarkupall <%>, напр. /setmarkupall 5")
        return
    try:
...
async def cmd_forceupdate_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Пересчитывает цены сразу для всех аккаунтов, без указания имени. Аккаунты на паузе пропускаются."""
    if not authorized(update):
        return
    accounts = context.bot_data["accounts"]
    if not accounts:
...
def main():
    ...
    app.add_handler(CommandHandler("setmarkupall", cmd_setmarkup_all))
    app.add_handler(CommandHandler("forceupdate", cmd_forceupdate))
    app.add_handler(CommandHandler("forceupdateall", cmd_forceupdate_all))
