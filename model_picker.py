...
def check_pump(sales: list, current_floor: float, threshold: float, tol_pct: float,
    ...
    одно — сколько модель стоит на самом деле. Если опорная цена выше порога,
    модель законная, даже когда прямо сейчас её выставили втрое дороже
    обычного; если ниже — не проходит, каким бы честным ни выглядел лот.

    Раньше в вердикт входило ещё и условие inflated: отсев срабатывал, только
    когда лот задран ОТНОСИТЕЛЬНО опорной цены. Условие умело лишь ослаблять
    проверку — модель с лотом чуть выше порога и опорной ценой чуть ниже него
    попадала в узкую щель между порогом и допуском и проходила как «честная».
    На живых данных так прошли 14 моделей из 478: у них дороже цены закупки с
    маржой уходило 37% сделок против 90% у остальных, а ценность превышала
    цену закупки в 1.07 раза против 1.72. Условие убрано, tol_pct остался
    только диагностикой.
    """
    ...
    verdict50 = "pump" if ref50 < threshold else "ok"
    # односторонне: current < ref_price — это просадка, а не памп.
    # На вердикт больше не влияет, остаётся в отчёте как диагностика.
    inflated = ref_price > 0 and current_floor > ref_price * (1 + tol_pct / 100)
    # берём модель, только если по реальным сделкам она дороже порога
