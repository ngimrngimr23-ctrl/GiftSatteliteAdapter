...
PREMIUM_STATE_FILE = "premium.json"  # фолбэк для локальной разработки
PREMIUM_REDIS_KEY = os.environ.get("REDIS_PREMIUM_KEY", "giftadapter:premium")
...
def load_premium() -> dict:
    """Замер надбавки за совпадение цвета: пары «совпадение -> доля цены»."""
    if _UPSTASH_ENABLED:
        return _load_from_upstash_key(PREMIUM_REDIS_KEY)
    return _load_from_file_path(PREMIUM_STATE_FILE)


def save_premium(data: dict):
    if _UPSTASH_ENABLED:
        _save_to_upstash_key(PREMIUM_REDIS_KEY, data)
        return
    _save_to_file_path(PREMIUM_STATE_FILE, data)
