...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "/watch [просадка%] [мин_цена] [макс_цена] [дней] [коротких_дней] [макс_просадка%] [all] — дозор: ловит модели, чей флор ушёл вниз. Находка обязана быть минимумом по сделкам сразу за оба срока, и не глубже [макс_просадка%] — глубже это обычно сломанные данные\n"
...
async def cmd_watch(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    params = scanner.WatchParams(
        ...
        # длинное окно и короткое — они ловят разное
        low_days=number(3, scanner.WatchParams.low_days),
        low_days_short=number(4, scanner.WatchParams.low_days_short),
        drop_max_pct=number(5, scanner.WatchParams.drop_max_pct),
